...
`/cache/history.json` is a version-1 JSON projection with `entries` containing
`id`, `path`, `tool` (Shell/Claude/Codex/Copilot/Hermes), and `opened_at`.
...
- IDs are at most 100 ASCII alphanumeric/hyphen bytes, tools are an enum, paths
  must be absolute without controls or parent traversal, schema must be version 1,
